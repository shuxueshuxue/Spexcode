...
When two visible nodes would otherwise show the same title or leaf name, the graph appends the shortest unique ancestor qualifier, joined with `/` (for example `parent/leaf`). The raw title remains unchanged for non-conflicting nodes. If identical titles share every human ancestor, the canonical node id is the final deterministic qualifier; backend identity and routes remain unchanged.
