...
export function resolveHarnessTargets(raw: unknown): HarnessTarget[] {
  ...
  for (const m of raw) {
    if (typeof m === 'string') {
      if (m === 'plugin')
        throw new Error(`spexcode.json "harnesses": a plugin target needs an EXPLICIT landing folder — write {"plugin":"<folder>"} (e.g. {"plugin":".adopter-a"}), not the bare string "plugin", because each host agent scans a different plugins dir.`)
      if (!KNOWN.includes(m))
      ...
      if (typeof folder !== 'string' || !folder.trim())
        throw new Error(`spexcode.json "harnesses": a {"plugin":…} target needs a NON-EMPTY folder string (e.g. {"plugin":".adopter-a"}) — each host agent scans a different plugins dir, so the folder must be explicit.`)
