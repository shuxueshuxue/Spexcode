...
writeFileSync(join(bin, 'git'), '#!/bin/sh\ncase "$*" in *"update-ref refs/spex-archive/"*) exit 73;; esac\nexec /usr/bin/git "$@"\n'); chmodSync(join(bin, 'git'), 0o755)
...
const env = { ...process.env, HOME: root, PATH: `${bin}:${process.env.PATH || ''}`, PORT: String(port), SPEXCODE_HOME: home, SPEXCODE_TMUX: tmuxSocket }
...
try {
  ...
  const closeBody = await close.text(); assert.equal(close.status, 409, closeBody)
  assert.match(closeBody, /could not publish refs\/spex-archive/)
  ...
  assert.match(run(project, ['show-ref', '--verify', `refs/heads/node/${id}`]), new RegExp(`refs/heads/node/${id}$`))
  assert.notEqual(spawnSync('git', ['-C', project, 'show-ref', '--verify', '--quiet', `refs/spex-archive/${id}`]).status, 0)
  console.log(JSON.stringify({ ok: true, id, closeStatus: close.status, closeBody, worktreeStillPresent: existsSync(worktree), recordStillPresent: existsSync(join(store, 'session.json')) }))
} catch (error) {
