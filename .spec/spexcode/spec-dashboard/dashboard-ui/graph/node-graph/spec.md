...
Collapse hides tiles, and it must not hide **activity**. Because only the focused spine is expanded, a session working three subtrees away is structurally invisible: its node exists in the tree but not on screen, and the closed tile standing in for it looks exactly like a quiet one. So a collapsed tile **advertises the activity inside it** — when any hidden descendant carries a pending overlay, the `▸N` tab **fills with the lead hidden author's colour** (the same seed→hue as that session's avatar and its own node rings, never a fifth colour) and carries **one dot per distinct session working inside**, capped with `+k`; its tooltip names each session and how many nodes it touches in there. A quiet closed branch and a busy closed branch therefore never look alike, just as a leaf and a closed branch never do. This is the aggregate the collapsed layout owes its reader: with sessions scattered across the tree, and **without expanding anything or moving the camera**, the board reads *how many places are in motion and under which branch each one sits*. Fill-versus-outline survives zoom that digits do not, so the lit tab is the signal and the counts are detail. The aggregation is **strictly descendant** — a node's own overlay already shows on its own tile as Row 1's glyphs, so the tab never restates it — and it is **not a camera driver**: activity anywhere lights a tab, while the camera still follows only the keyboard ([[keyboard-nav]]). Lighting the ancestor is what makes the drill-down affordable at real scale: the reader is told where to drill instead of being made to search, so the short root column stays the whole interface rather than a thing to escape from.
