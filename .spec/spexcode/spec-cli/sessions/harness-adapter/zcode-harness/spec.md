...
z-code discovers Claude-compatible command hooks from `.zcode/settings.json`. Its shim binds the five lifecycle events that SpexCode needs — `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop` — with each command dispatching the baked `zcode` harness id. Materialization proves that wiring, but it is not runtime proof: the generated `Stop` command must consume `zcode` as its harness id, select the current tree's manifest, and execute the blocking Stop gate against the governed record. z-code has no `Notification` or `StopFailure`, so the existing Claude-only idle state is unavailable and no substitute event is fabricated. A zcode-only seed therefore omits those unreachable `idle` and `session-fail` hook nodes and their scripts; a multi-harness seed keeps either when another selected adapter declares its event. The payload is Claude-shaped, so the shell mirror routes `zcode` through the existing Claude-family parser rather than adding a second parser.

z-code has a native slash-command system, but this adapter's row returns no `slashCommands`. Consequently the five template `commands/` nodes produce no zcode command artifact today. They remain seeded: absent delivery is a visible adapter gap whose repair entrypoint is the `zcode` row's own `slashCommands`, not evidence that the commands are unnecessary or permission to delete them. A repair must prove the commands appear in z-code's actual command surface and are invocable; it is separate from lifecycle-event seed selection.
