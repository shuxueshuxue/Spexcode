...
The record's existing `name` is the one human display override: creation may set it once from an explicit
name input, and rename later replaces or clears that same field. It affects only the shared label/title projection;
