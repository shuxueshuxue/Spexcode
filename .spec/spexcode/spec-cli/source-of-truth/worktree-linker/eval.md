...
scenarios:
  - name: foreign-base-no-phantom-ops
    description: >
      ...
      .spec tree — the adopter-a CR-review shape: the branch is an MR head plus a commit restoring .spec) while
    ...
    expected: >
      ...
      proposals intact.
    tags: [backend-api]
    code: packages/spec-core/src/git.ts#worktreeSpecDelta
    related: [packages/spec-core/src/layout.ts, spec-cli/src/git.test.ts]
---
