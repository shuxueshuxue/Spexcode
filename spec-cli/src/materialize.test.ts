...
test('one per-tree materialize projection survives a sibling pass across divergent config and plugins', { skip: !gitAvailable() && 'git not available' }, () => {
  ...
  g('add', '.gitignore')
  assert.equal(g('status', '--short'), '', 'legacy common ignore keeps the not-yet-upgraded main tree clean')
  ...
  spex(wt, 'materialize')
  assert.equal(g('status', '--short'), '', 'upgrading the linked tree cannot expose a legacy main tree')
  ...
  assert.ok(existsSync(join(proj, 'CLAUDE.md')) && !existsSync(join(proj, 'AGENTS.md')), 'main keeps only its own local selection')
