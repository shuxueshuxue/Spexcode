...
status: active
hue: 260
desc: Where the dashboard proxies /api — a per-project `.spec/spexcode.json` default, overridable by the API_URL env.
code:
...
2. `.spec/spexcode.json`'s `dashboard.apiUrl` — the **per-project default**, found by walking up from cwd.
...
`.spec/spexcode.json` found by walking up from the dashboard's own cwd — so it names the project's backend
