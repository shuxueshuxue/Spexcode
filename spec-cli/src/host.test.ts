...
test('directory browse reports folder state; explicit setup initializes Git then the real SpexCode CLI before cataloging', async () => {
  ...
  mkdirSync(plain)
  writeFileSync(join(plain, 'notes.md'), 'created before SpexCode\n')
  ...
  writeFileSync(join(existingUnborn, 'draft.txt'), 'retry this project\n')
  execFileSync('git', ['init', '-q', '-b', 'master'], { cwd: existingUnborn })
  const repaired = await withGitIdentity(() => addKnownProjectWithSetup(existingUnborn))
  ...
  assert.match(execFileSync('git', ['-C', existingUnborn, 'log', '-1', '--format=%s'], { encoding: 'utf8' }), /^chore: 初始化项目\n$/)

  const historical = join(parent, 'historical')
  mkdirSync(historical)
  await withGitIdentity(async () => {
    execFileSync('git', ['init', '-q', '-b', 'master'], { cwd: historical })
    writeFileSync(join(historical, 'tracked.txt'), 'already committed\n')
    execFileSync('git', ['add', '--', 'tracked.txt'], { cwd: historical })
    execFileSync('git', ['commit', '-qm', 'existing history'], { cwd: historical })
  })
