...
test('production seed carries the high-risk measurement and Codex multi-file invariants', () => {
  ...
  assert.match(repair, /A, before editing:[\s\S]*?--fail/)
  assert.match(repair, /B, after editing:[\s\S]*?commit the verified[\s\S]*?--pass/)
  assert.match(repair, /reading's `codeSha` must be that commit/)
