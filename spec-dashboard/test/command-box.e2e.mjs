...
assert.ok(slashRows.some((row) => row.includes('/stop')), slashRows.join('\n'))
...
await page.locator('.si-command-send').click()
await page.locator('.si-command-box .si-action-outcome.queued').waitFor({ state: 'visible' })
assert.match((await page.locator('.si-command-box .si-action-outcome.queued').textContent()) || '', /waiting for the terminal transport/i)
assert.equal(await input.inputValue(), draft)
...
for (const probe of [issueComposer]) {
...
await browser.close()
writeFileSync(join(OUT, 'timeline.json'), JSON.stringify({ v: 2, axis: 'time', events }, null, 2))
writeFileSync(join(OUT, 'result.json'), JSON.stringify({ session: SESSION, sidebar, widened, persisted, reset, empty, grown, inputs, issueComposer }, null, 2))
