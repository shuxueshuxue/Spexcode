...
One graph build already reconciles resident local/forge Issues. At successful completion it atomically
publishes that full source population to process memory, replacing the previous snapshot as a unit. The
first `/api/issues` request waits for that first successful publication; once a snapshot exists, a request
...
row snapshot permanently behind, without product-level polling. A later page revision/poll observes the
replacement generation. A sessions-only graph splice leaves the snapshot valid because session presence is
joined separately at request time.
...
second datastore: git/spec/issue sources remain authoritative and graph invalidation remains the one
refresh trigger.
