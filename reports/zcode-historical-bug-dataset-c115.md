# z-code 严重历史 Bug 数据集：2026-07-24 快照

## 快照边界与证据范围

- 历史源码基线：z-code commit `5eae8f0dcb`。本文件描述该基线时已修复事故与当时的模型边界，不代表后续 HEAD 的当前能力清单。
- 文件路径全部是 z-code 仓库相对路径；每条历史证据固定到完整或唯一前缀 commit，不依赖 checkout 位置或宿主机状态。
...
| ZB-04 | `apps/zcode-cli/packages/adapters/src/model/registry.ts@5e14d8f1:491-510`; `apps/zcode-cli/docs/design/v2/model/stream-error-retry.md` | `registry.test.ts`; `runner-provider-business-network.test.ts` | `regression` |
...
| ZB-07 | `packages/services/src/process/processTreeTerminator.ts@6bc9a7c8:137-205,335-339` | `processTreeTerminator.test.ts`; `zcodeStdioTransport.test.ts` | `regression` |
| ZB-08 | `apps/zcode-cli/packages/core/src/permission/broker.ts@d0a989d4:56-92`; incident note in commit | `apps/zcode-cli/packages/core/tests/permission-broker.test.ts:98-148` | `observed`, `regression` |
...
| ZB-13 | `apps/zcode-cli/packages/core/src/subagent/runner.ts@603f06fd:182-300,1104-1118`; `docs/subagent-timeout-policy.md` | `subagent-explore.test.ts:167-218,286-340`; tool-event tests | `observed`, `regression` |
...
| ZB-21 | `packages/services/src/zcode-session/zcodeSessionService.ts@48cf3780:243-332,559-582` | `zcodeSessionService.test.ts:177-668` | `observed`, `regression` |
