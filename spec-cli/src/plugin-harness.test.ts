...
test('emitPlugin writes a self-contained Claude-plugin bundle under <folder>/plugins/spexcode', () => {
  ...
  emitPlugin(proj, '.adopter-a', RENDER)
  const bundle = pluginBundleDir(proj, '.adopter-a')
  assert.equal(bundle, join(proj, '.adopter-a', 'plugins', 'spexcode'))
  ...
  // hooks.json: Claude/adopter-a shape, every event → dispatch.sh with the `plugin` harness id via ${CLAUDE_PLUGIN_ROOT}
  ...
  // contract → additionalContext, JSON-encoded at render time (arbitrary prose, quotes + newlines, survives)
  ...
  assert.equal(ctx.hookSpecificOutput.additionalContext, RENDER.contract)
...
test('emitPlugin is idempotent — re-emitting overwrites in place', () => {
  ...
  emitPlugin(proj, '.claude', RENDER)
  emitPlugin(proj, '.claude', { ...RENDER, version: '1.2.3' })
...
test('cleanPlugin removes OUR bundle only — identity-gated on plugin.json name', () => {
  ...
  emitPlugin(proj, '.adopter-a', RENDER)
  assert.ok(existsSync(pluginBundleDir(proj, '.adopter-a')))
  cleanPlugin(proj, '.adopter-a')
  assert.ok(!existsSync(pluginBundleDir(proj, '.adopter-a')))
})
...
test('cleanPlugin is a no-op when nothing is there', () => {
  ...
  cleanPlugin(proj, '.adopter-a')   // must not throw
  assert.ok(!existsSync(pluginBundleDir(proj, '.adopter-a')))
})
