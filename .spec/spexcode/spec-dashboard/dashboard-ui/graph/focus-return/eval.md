...
scenarios:
  ...
  - name: timeline-selection-keeps-composer-sink
    ...
    description: >
      ...
      while dragging across at least three words, and triple-click the note. Continue the click sequence
      through details four and five, then toggle the prompt details summary while the word Range remains
      visible. Read the actual CSS Custom Highlight Range text and word count after each gesture, copy the
      multi-word Range, start editing from a middle textarea caret, and make a plain click. Repeat at
    ...
    expected: >
      ...
      landing word, and LINE matches the complete note through details four and five. Ctrl/Cmd+C preserves and
      copies the full highlighted Range. A summary toggle preserves its current Range while performing its native
      action. The first editing key clears only that custom Range and applies once through the textarea's
      continuously native caret. A direct composer press clears the Range before native caret placement. A plain
      timeline click creates no Range, and interactive descendants still perform their native click action without
      stealing the composer sink.
    related:
