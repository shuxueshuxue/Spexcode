...
- **`settings` → the agent-facing RUNTIME-SETTINGS manual.** SpexCode's own settings are self-documenting
  ...
  owners. Its launcher table mirrors [[launcher-select]]: the seeded Codex profile uses `codex --yolo`, while
  Claude, OpenCode, and Pi keep their ordinary commands; the independent [[opencode-headless]] profile uses
  `opencode --auto` because its terminal-free runtime cannot ask for approval. Other permission models remain
  authored profiles or local overrides. The sessions section names the worker cap's default, precedence,
