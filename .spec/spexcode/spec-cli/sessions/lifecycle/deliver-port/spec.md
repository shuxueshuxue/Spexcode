...
verified fix). So the trunk driver's admission does not invent a new proof artifact; it points at the
evidence the session already handed the reviewer through `spex session files add`, and that pair rides along
as the argument that justifies the merge. This stays a *reference*, not a copy: the session's files remain
the single source of that evidence, exactly as destination-single keeps delivery from duplicating what a
mirror observes. A non-repair delivery (new intent, no prior failure) has
