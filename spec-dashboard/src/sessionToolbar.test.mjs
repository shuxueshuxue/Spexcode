...
test('session tree drag moves the whole row and can detach a nested session', () => {
  ...
  assert.match(source, /const SESSION_DRAG_GHOST_SCALE = 0\.5/)
  ...
  assert.match(css, /\.si-session-drag-ghost \{[\s\S]{0,520}z-index: 52;[\s\S]{0,520}pointer-events: none; transform: translate\(10px, -8px\) rotate\(-1deg\) scale\(\.5\); transform-origin: top left;/)
