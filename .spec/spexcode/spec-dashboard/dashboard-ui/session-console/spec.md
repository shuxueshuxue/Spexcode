...
uses the dashboard-only primary+Shift+E chord for the selected row's existing [[session-nesting]] fold:
**⌘/Ctrl+Shift+E toggles a visible parent**, including while xterm or a composer owns focus. It claims the
chord only when that row's disclosure can change; a leaf keeps its native input key, arrow keys retain their
normal editing semantics, and the action never changes selection or session data. **⌥+N** reaching the New Session composer is no longer this console's own
