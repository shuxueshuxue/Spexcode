...
test('shared-runtime projection uses live adapter refs and fail-closed process identity', async () => {
  ...
  try {
    ...
    writeFileSync(join(duplicateDir, 'session.json'), `${JSON.stringify(record(duplicate, 'thread-sibling'), null, 2)}\n`)
    await assert.rejects(() => assertSessionStopSafe(target, { session: target, harness: 'codex' }), /loaded thread.*without one exact governed session owner.*thread-sibling/)
    ...
    await assert.rejects(() => assertSessionStopSafe(target, { session: target, harness: 'codex' }), /no readable owner PID.*could not prove its live references/)
    ...
    writeFileSync(codexAppServerPid(root), '99999999\n')
    await assert.rejects(() => assertSessionStopSafe(target, { session: target, harness: 'codex' }), /unreadable process-start identity.*could not prove its live references/)
    writeFileSync(codexAppServerPid(root), `${sharedRoot.pid}\n`)
    probe = governedProbe(true)
    ...
    assert.equal(governed?.reclaim?.eligible, false, 'an unowned loaded thread blocks every session stop even when the target itself is terminal')
