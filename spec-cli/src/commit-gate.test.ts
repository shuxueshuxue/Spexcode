...
test('cherry-pick and rebase reject replayed debt and continue after an atomic spec repair', () => {
  ...
  const cherryBefore = cherry.git('rev-parse', 'HEAD')
  const pick = cherry.runGit({ SPEXCODE_GATE_TRACE: '1' }, 'cherry-pick', picked)
  assert.notEqual(pick.status, 0, `cherry-pick replayed debt without invoking the gate:\n${pick.stdout}${pick.stderr}`)
  assert.equal(cherry.git('rev-parse', 'HEAD'), cherryBefore)
  assert.match(`${pick.stdout}${pick.stderr}`, /anchor-drift/)
  assert.ok(existsSync(join(cherry.root, '.git', 'CHERRY_PICK_HEAD')), 'rejected cherry-pick lost sequencer recovery state')
  writeFileSync(join(cherry.root, '.spec', 'project', 'calc', 'spec.md'), NODE.replace('The calculation contract.', 'The calculation contract, revised during cherry-pick.'))
  cherry.git('add', '.spec/project/calc/spec.md')
  const cherryContinue = cherry.runGit({ GIT_EDITOR: 'true' }, 'cherry-pick', '--continue')
  assert.equal(cherryContinue.status, 0, `cherry-pick --continue did not recover:\n${cherryContinue.stdout}${cherryContinue.stderr}`)
  assert.equal(cherry.lint().status, 0)
  ...
  const rebasedBranch = rebased.git('rev-parse', 'refs/heads/rebased-side')
  const rebase = rebased.runGit({ SPEXCODE_GATE_TRACE: '1' }, 'rebase', 'upstream')
  assert.notEqual(rebase.status, 0, `rebase replayed debt without invoking the gate:\n${rebase.stdout}${rebase.stderr}`)
  assert.equal(rebased.git('rev-parse', 'refs/heads/rebased-side'), rebasedBranch)
  assert.match(`${rebase.stdout}${rebase.stderr}`, /anchor-drift/)
  writeFileSync(join(rebased.root, '.spec', 'project', 'calc', 'spec.md'), NODE.replace('The calculation contract.', 'The calculation contract, revised during rebase.'))
  rebased.git('add', '.spec/project/calc/spec.md')
  const rebaseContinue = rebased.runGit({ GIT_EDITOR: 'true' }, 'rebase', '--continue')
  assert.equal(rebaseContinue.status, 0, `rebase --continue did not recover:\n${rebaseContinue.stdout}${rebaseContinue.stderr}`)
  assert.equal(rebased.lint().status, 0)
})
