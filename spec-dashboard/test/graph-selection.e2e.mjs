...
try {
  ...
  await page.waitForFunction(() => location.hash.startsWith('#/sessions/new?seed='), null, { timeout: 10_000 })
  assert.match(await page.evaluate(() => location.hash), /^#\/sessions\/new\?seed=/)
  await page.locator('.si-input').waitFor({ state: 'visible', timeout: 30_000 })
  await page.waitForFunction((ids) => ids.every((id) => document.querySelector('.si-input')?.value.includes(`[[${id}]]`)), selectedIds, { timeout: 30_000 })
