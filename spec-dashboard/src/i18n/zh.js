export default {
  ...
  session: {
    ...
    // 搁置面（[[archive]]）——措辞不暗示"停止"，因为搁置什么都不停
    viewGroup: '会话列表视图',
    shelfShow: '搁置架——你暂时放下的会话',
    shelfHide: '回到工作列表',
    shelfEmpty: '搁置架是空的',
    shelfMsg: '★ 已搁置——只是放到一边，没有停止也没有删除。',
    shelfSubLive: 'agent 仍在运行，工作树原封不动。想用的时候随时取回列表。',
    shelfSubOffline: '进程已退出，但工作树和对话都还在——先取回列表，要继续做的时候再重新启动。',
    shelfRestore: '★ 取回列表',
    msgError: '⚠ 未送达 — 重试',
    ...
    cmd: {
      ...
      archiveTitle: '搁置此会话（移出列表；不停止任何东西）',
      archiveDesc: '归档 —— 把它移出列表。agent 继续运行、工作树保留；想连进程一起收回就再跑 /stop',
      unarchiveTitle: '把此会话取回列表',
      unarchiveDesc: '取消归档 —— 回到工作列表',
