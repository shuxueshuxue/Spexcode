...
| ARCH-CROSS | 无人引用的 `pass-zswarm.log`，与文档点名的原始日志逐字节相同 | `spikes/adopter-api/pass-zswarm.log` 与 `spikes/adopter-api/fail-first-zswarm.log` 内容同为一行 `no executable proof available at this base: repository has no production ZSwarm importer`（`cmp` 相同，同一提交 `f8023bad6` 落地）；`docs/session-adopter-cutin-plan.md:45` 只点名 `fail-first-zswarm.log` 保存原始结果；全仓库对 `pass-zswarm.log` 的引用为 0；`docs/session-platform-m2-integration.md:33-38` 的 immutable 锚点清单里两者都不在 | 记录 ZSwarm 在此 base 上无可执行证明 | 状态本身该留（见下一条 ARCH-CHECK）。冗余的是这个文件：它是原始日志的无引用副本，而 `pass-` 前缀是"看起来已 GATED"这一误读的唯一来源。缺的从来不是退出码——77 已被记了三处：`docs/session-adopter-cutin-plan.md:15`、`:45`，以及 `docs/session-platform-m2-integration.md:30` 那行三 adopter 单行判定（`self-launch PASS · Spex governed PASS · ZSwarm exit 77`），它本身就是这批证据现成的单一结构化结果 | 撤掉这份无引用副本，让 ZSwarm 只剩文档点名的那一份。备选路"在 `docs/session-adopter-cutin-plan.md:45` 补一句点明它是同字节副本"已被检查并放弃：核对 `docs/session-platform-m2-integration.md:70-71` 后确认第 1 条只保护原始失败证据，对无引用的重复文件没有要求。两条路都不新增文件、不新增约定，采用前者 | 低。执行方是 adopter-api/evidence owner，不是 gate owner，也不是本审查 lane——本文不执行删除。原始日志 `fail-first-zswarm.log` 一字节不许动，`spikes/adopter-api/zswarm-contract.mjs` 与 `docs/session-platform-m2-integration.md:30` 的判定行同样不动。删后语义：撤掉这份副本不破坏任何一条——三态仍由 `:30` 与仍在的原始日志承载；但两份都删会让 NOT MEASURED 退化成"没跑过"，与 GATED/UNGATED 不可分 |
...
本表有一条经父侧复核后改写过：ZSwarm 证据那一行原本建议"补一个记录退出码 77 的 sidecar"。复核要求确认是否存在
更简单的单一结构化结果，答案是有，而且已经在仓库里——`docs/session-platform-m2-integration.md:30` 的三 adopter
单行判定已带 `exit 77`，`docs/session-adopter-cutin-plan.md:15` 与 `:45` 又各记一次。`.exit` 这个约定在
`spikes/legacy-sabotage/` 确实存在（12 份），所以原建议不算发明新约定，但它会是同一事实的第四份副本，正是本文
自己要找的那类缺陷。改写后的最小建议因此从"增一个文件"变成"撤一个无人引用的副本"，方向反了过来。

这一条随后被结清：父侧核对 `docs/session-platform-m2-integration.md:70-71`，确认第 1 条只保护原始失败证据、并未要求
保留无引用的重复文件，因此走撤副本那条路，由 adopter/evidence owner 执行——不新增 sidecar，不动
`fail-first-zswarm.log`，也不由本审查 lane 执行删除。把结论写回表里而不是留着那个条件分支，是因为一个已被裁定的
分支继续挂在清单上，就是本文第一轮判为 CROSS 的那类账：已经定了，却仍记成待定。

两处修法的归属由父侧裁定，记在此处以免落错人手：L09 的两个问题（补 `runtime_*` 字段选择器、更正
`runtime-session.ts:1-239` 的范围）属 legacy deletion gate 的正确性，由 gate owner 以 additive commit 承接并重跑
gate 与 lint，本审查 lane 不直接改；ZSwarm 证据那条属 adopter-api/evidence owner，不归 gate owner。
