...
Resolution is explicit session override, then the saved default, then Terminal. Updating the default never
rewrites session overrides. Updating a session override publishes the new value to the currently mounted console
immediately. The storage key includes the current project scope, so one browser's choice for project A cannot
