...
contract through the dedicated `@spexcode/l0/identity` package entry, never the Node core entry. The existing `dashboard.icon` field stores the chosen human-readable value; no catalog result is
