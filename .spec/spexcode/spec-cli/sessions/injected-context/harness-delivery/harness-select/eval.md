...
scenarios:
  - name: l0-only-adoption
    tags: [cli]
    description: >-
      Run `spex init --harness none` in a fresh git repo. Inspect the whole tree, the planted .spec/spexcode.json,
    ...
    expected: >-
      .spec/spexcode.json carries "harnesses": [] and the tree gains ONLY .spec/ and .spec/spexcode.json — no .claude,
  ...
  - name: illegal-harness-set-fails-loud
    tags: [cli]
    description: >-
      In a fresh git project, set .spec/spexcode.json `harnesses` to an illegal value and run `spex materialize`:
      first ["claude", {"plugin": ".adopter-a"}] (plugin + native), then ["plugin"] (plugin, no folder).
    expected: >-
  ...
  - name: missing-selection-fails-loud
    ...
    description: >-
      ...
      adopted project, delete the `harnesses` field from .spec/spexcode.json and run `spex materialize`.
    expected: >-
      Both fail loud, non-zero: init aborts BEFORE writing anything (no .spec, no .spec/spexcode.json) with an
  ...
  - name: init-stamps-the-choice
    tags: [cli]
    description: >-
      Run `spex init --harness codex` in a fresh git project and inspect the planted .spec/spexcode.json and tree.
    expected: >-
      .spec/spexcode.json carries "harnesses": ["codex"] and ONLY a codex launcher (defaultLauncher "codex");
  ...
  - name: selection-edit-self-heals
    tags: [cli]
    description: >-
      Adopt with `harnesses` ["claude","codex"] (both natives delivered), then narrow .spec/spexcode.json `harnesses` to
  ...
  - name: checkout-method-independent-selection
    ...
    description: >-
      Put a codex-only .spec/spexcode.json on a candidate commit while main carries a different valid selection.
...
project whose .spec/spexcode.json carries a `harnesses` set. Two things must hold: an ILLEGAL set fails loud with an
