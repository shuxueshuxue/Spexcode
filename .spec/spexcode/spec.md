...
optimizer reads), and **session-core** (the reusable file-backed session communication protocol consumed
by spec-cli and external runtimes).
...
is refused at this altitude). **L1 — the agent-army substrate**: `session-core` implements the durable
communication protocol and `spec-cli` composes it with lifecycle and harness control to drive workers on top
of that asset, the brick a CI, a review pipeline, or a software factory builds with, so it
