...
code:
  - spec-dashboard/src/FocusPanel.jsx#FocusPanel
  - spec-dashboard/src/FocusPanel.jsx#ScenarioRow
related:
...
(the eval loss targets, with each one's satisfaction status). It makes Issues and Scenarios **equal
...
and **scenarios** (internal loss targets, satisfied/outstanding — [[eval-score-badge]]). Both are *execution*
...
`scenarioStates` from [[eval-score-badge]]) so a **never-measured** scenario still appears — it is a unit of
...
scopes its own freshness — the **files it tracks** (per-scenario `code`, [[eval-core]]). The whole row is a
...
tabs ([[eval-tab]]) remain reference panes for the focused node. Editing or creating scenarios is a
