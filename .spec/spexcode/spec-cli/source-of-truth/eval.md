...
scenarios:
  - name: derivation-from-git
    ...
    description: >-
      ...
      revert that edit in a second one-parent commit, and merge the net-TREESAME branch; (4) create a second
      node's spec.md only in a merge whose parents both lack that file; (5) inspect the repo for any persisted
      derivation state beside `.spec`.
    expected: >-
      ...
      can hide them behind the TREESAME merge; the merge-created spec.md is that second node's first version;
      and no datastore/hash/index file exists beside the spec tree — delete nothing, recompute everything.
---
