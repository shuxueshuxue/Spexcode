...
scenarios:
  ...
  - name: codex-launcher-policy-reaches-backend-thread
    ...
    expected: >-
      ...
      can stop for an approval prompt. The failure this locks: a `--yolo` flag present only on the later
      `--remote resume` command leaves the already-created backend thread at Codex defaults (`on-request` plus
      `workspace-write`), so the displayed launch command promises YOLO while the running agent still asks
      the human to approve tools.
  ...
  - name: claude-background-fork-rendezvous-auth
    tags: [backend-api, cli]
    code:
      - spec-cli/src/harness.ts#deliverViaClaudeRendezvous
      - spec-cli/src/harness.ts#claudeForkTransport
    description: >-
