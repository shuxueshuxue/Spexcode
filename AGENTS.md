...
Four deployments, two supply chains. (Correction to folk memory: gugu/adopter-a do NOT pull from the npm
registry — both install a locally-built tarball via `npm i -g`; adopter-a's mbp cannot even reach GitHub.)
...
## Supply chain B — local tarball to npm-global (Macs): gugu (macmini) + adopter-a (mbp)
...
3. **adopter-a checkout too** (mbp keeps a source checkout at `~/spexcode`, GitHub-blocked):
