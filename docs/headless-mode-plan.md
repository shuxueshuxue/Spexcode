...
> 调查覆盖 mbp/adopter-a 手搓 `codex-spex-exec` 的机制与双跑实证、claude `-p` 下 stop-gate
...
- **舰队**:随常规发版走(supply chain A/B 不变);gugu/adopter-a 想启用只需在 `spexcode.local.json` 加一行 `headlessCmd`。
