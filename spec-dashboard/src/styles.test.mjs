...
test('tab widths follow content before wrapping and keep the active close affordance', () => {
  ...
  assert.match(css, /\.tab\s*\{[^}]*flex:\s*0 1 auto;[^}]*width:\s*auto;[^}]*min-width:\s*80px;[^}]*max-width:\s*240px;/s)
  assert.match(css, /\.tab\.on\s*\{[^}]*min-width:\s*112px;[^}]*background:\s*var\(--paper\);[^}]*border-right-color:\s*transparent;[^}]*border-radius:\s*var\(--radius\)/s)
  assert.match(css, /\.tab:not\(\.on\):hover\s*\{[^}]*background:\s*var\(--panel2\);/s)
  assert.match(css, /\.tab-x\s*\{[^}]*flex:\s*0 0 0;[^}]*width:\s*0;[^}]*overflow:\s*hidden;/s)
  assert.match(css, /\.tab\.on \.tab-x, \.tab:hover \.tab-x\s*\{[^}]*flex-basis:\s*24px;[^}]*width:\s*24px;[^}]*opacity:\s*1;/s)
  assert.match(css, /\.tab\.tab-closing\s*\{[^}]*animation:\s*tab-out var\(--dur-tab\)/s)
