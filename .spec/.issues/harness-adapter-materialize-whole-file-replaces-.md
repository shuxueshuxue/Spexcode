...
**Observed (adopter-a, fresh `node/cr-listener-3e0d` worktree)**: after materialize, tracked `.claude/settings.json`
...
is not specific to adopter-a or to the 3e0d lane. 3e0d is restoring exact HEAD bytes only and the fix must not be
