...
export function recordStatus(id: string, status: SessionLifecycle, proposal: SessionProposal | null, note: string | null): void {
...
export type SessionTurn = Readonly<{ token: string; acceptedAt: string }>

export function currentHumanTurn(id: string): SessionTurn | null {
  const evs = timelineEvents(id)
  for (let i = evs.length - 1; i >= 0; i--) {
    const e = evs[i]
...
export function timelineTail(id: string, limit = 500): TimelineEvent[] {
