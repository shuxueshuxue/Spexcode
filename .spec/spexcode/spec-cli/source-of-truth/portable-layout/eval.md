...
scenarios:
  - name: committed-plus-local-overlay
    description: >
      ...
      gitignored .spec/spexcode.local.json override, then make one config file malformed and re-run.
    expected: >
      Both checkout methods keep naming `staging`; the ordinary switch never redefines `node/x` as trunk.
      Resolution follows local overlay > committed .spec/spexcode.json > conventional `main`. A present malformed
