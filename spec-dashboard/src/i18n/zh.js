export default {
  ...
  plugins: {
    ...
    installed: '声明在这里 —— 活着的会话实际在跑什么',
    matching: '跑着声明的契约',
    differing: '跑的是别的',
    unmaterialized: '一个钩子都不跑 —— 没有 Stop 闸',
    ofTrees: ({ trees }) => `共 ${trees} 个活会话`,
    liveAria: ({ matching, trees }) => `${trees} 个活会话中有 ${matching} 个跑着声明的钩子契约`,
    diffExtra: '多出的绑定',
    diffMissing: '缺失的绑定',
    order: '顺序',
    blocks: '阻塞',
    ...
    alwaysOn: '始终生效',
    alwaysOnNote: '按名称顺序整段折进每个启动的 agent 的契约块。',
    invoked: '按需调用',
    invokedNote: 'agent 自己调的 skill，人从 / 菜单里选的 command —— 或者两者都是。',
    bothSurfaces: 'skill + command',
    unused: ({ names }) => `${names} 这个面在词表里，但现在空着`,
