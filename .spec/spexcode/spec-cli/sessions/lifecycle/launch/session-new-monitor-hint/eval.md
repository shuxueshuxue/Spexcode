...
scenarios:
  - name: session-verb-help-is-specific
    ...
    expected: >
      ...
      selector/write entries keep the shared SEL grammar/project-bound safety notes; `done --propose nothing`
      identifies its no-write trap and names its four real destinations; `ask` names the human reply,
      direction, or decision a reported finding/recommendation may await. Every entry points back
