...
try {
  ...
  console.error('health ok', port, project, home)
  const close = await fetch(`http://127.0.0.1:${port}/api/sessions/${id}/close`, { method: 'POST', signal: AbortSignal.timeout(10000) })
  const closeBody = await close.text(); assert.notEqual(close.status, 200, closeBody)
  assert.equal(existsSync(worktree), true)
  assert.equal(existsSync(join(store, 'session.json')), true)
  let refPresent = true; try { run(project, ['show-ref', '--verify', `refs/spex-archive/${id}`]) } catch { refPresent = false }
  assert.equal(refPresent, false)
