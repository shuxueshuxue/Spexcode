...
if [ -f "$taught" ]; then
  printf '{"decision":"block","reason":"undeclared stop — declare the ONE true state as your LAST call: `%s session <done --propose merge (review; ONLY clickable merge)|close (close-pending; no posted artifact waiting)|park (parked; real wake-up + next action) / ask (asking; human reply or posted artifact review)>`. `done --propose nothing` is a trap: it writes no state and names these choices. Conditions: `%s help session`."}\n' "$S" "$S"
  exit 0
fi
