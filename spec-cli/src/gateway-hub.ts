...
export function startHubGateway(opts: HubOpts): http.Server {
  ...
  const handler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
    ...
    if (pm) {
      ...
      // fallback under the project prefix. API/SSE/health fetches, posted-web frames, and WS upgrades keep
      // the auth gate and proxy to their backend untouched.
      if (req.method === 'GET' && !sub.startsWith('/api') && !sub.startsWith('/web/') && ext.fallback) {
        if ((req.headers.accept ?? '').includes('text/html')) return ext.fallback(req, res, '/')
        if (sub !== '/') return ext.fallback(req, res, sub)
      }
