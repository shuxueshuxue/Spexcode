export default {
  ...
  plugins: {
    ...
    installed: 'Declared here — what the live sessions are actually running',
    matching: 'run the declared contract',
    differing: 'run something else',
    unmaterialized: 'run no hooks at all — no Stop gate',
    ofTrees: ({ trees }) => `across ${trees} live session${trees === 1 ? '' : 's'}`,
    liveAria: ({ matching, trees }) => `${matching} of ${trees} live sessions run the declared hook contract`,
    diffExtra: 'extra binding',
    diffMissing: 'missing binding',
    order: 'order',
    blocks: 'blocks',
