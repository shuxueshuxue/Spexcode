...
2. **正文是活文档。** 永远描述当前意图,原地重写。spec 正文里禁止出现 changelog 标题(linter 强制),
   历史 git 已经记了。
3. **spec 和代码一起落地。** 一次改动就是一个 commit,同时更新 `spec.md` 和它所解释的代码。代码
   要是脱开 spec 单独动了,linter 会把这个文件标成 drift,一直标到 spec 跟上为止。

## 优化循环

上面这几件东西合起来是一个循环。spec 是损失函数:定义你要什么,这一半由人拍板。commit 是优化器。
**yatsu** 是测量子系统,负责评估:量出当前行为离 spec 还有多远,分数的历史照样存在 git 里。

<img src="docs/readme-loop.zh.png" alt="优化循环示意">

这个框架也决定了人平时站在哪。没有人逐行审神经网络的权重,大家看的是 loss 曲线。对 agent 写的
代码同理:两次 merge 之间,注意力放在 spec 和 eval 读数上,真正的 diff 留到 merge 的时候读。
...
merge 并停下。worker 不自己 merge,合并是管理者的决定;你按下 merge 时,实际的 git merge 由这个
...
自己实现的那套 worker/manager 循环落地。你装到的看板就是造它用的那块。它还年轻,有几根毛刺是有
名有姓的:`spex session new --help` 不会打印帮助,而是真的创建一个叫 `--help` 的会话(派工请用
`spex new`);altitude lint 会在你学会在乎之前就开始唠叨 spec 写得太长。首次公开介绍发在
