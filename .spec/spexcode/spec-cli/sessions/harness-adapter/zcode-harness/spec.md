...
z-code has a native slash-command system, but this adapter currently returns no `slashCommands` rows at `harness.ts:2982`. Consequently the five template `commands/` nodes produce no zcode command artifact today. They remain seeded: absent delivery is a visible adapter gap with `harness.ts:2982` as its repair entrypoint, not evidence that the commands are unnecessary or permission to delete them. A repair must prove the commands appear in z-code's actual command surface and are invocable; it is separate from lifecycle-event seed selection.
