...
| L09 | `packages/session-core/src/runtime-session.ts` mixed bridge | **SUPERSEDED BY MEASUREMENT (M5 reconnaissance).** The external importer is real and now cited: ZCode's swarm runtime declares `"@spexcode/session-core": "0.6.7"` at `apps/zcode-cli/packages/bootstrap/package.json:24` and imports **all six** bridge exports — `drain`, `publishRuntimeSessionState`, `readRuntimeSession`, `registerRuntimeSession`, `runtimeSessionNotification`, `runtimeSessionChildren` — at `apps/zcode-cli/packages/bootstrap/src/swarm-session-protocol.ts:1-8` (measured on z-code `codex/swarm-five-integrated` @ `b9b3fa701`, local checkout `/home/jeffry/zcode`). Its own spec node `.spec/zcode/swarm-orchestration/session-protocol/spec.md` states the arrangement in prose: swarm sessions use `@spexcode/session-core` for the session record, timeline, parent watch, pending queue, cursor and their locks. The earlier reading — no production importer in THIS repository, external ZSwarm use unproven — was true of this repository and false about the world; the consumer was simply in another repository nobody had opened. **Consequence: removing the bridge before that adopter migrates breaks a live external product**, so the M8 demolition plan and the simplification audit's zero-cost reading of this module are both blocked on M5. | One module writes session JSON, parent/watchers, status timeline and pending delivery under both lock families. **Shared writer: yes**, deliberately spanning L01/L02/L04/L05/L06/L07. | Protocol package plus adopter topology/runtime composition. Freeze/remove the mixed root contract once the M5 installed-adopter proof uses split APIs; remove source and Spex calls at **M6**, package residue at **M8**. | Install a clean packed external consumer. The old bridge export must be absent while registration, lifecycle projection, topology notification and dequeue work through their new owners; static imports of `runtime-session` are zero. |
