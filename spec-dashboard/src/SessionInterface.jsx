...
export default function SessionInterface({ sessions, specs = [], focusNode, open, searchOpen = false, sel, setSel, seed, onSeedConsumed, onClose, onPickSession, onOpenSearch, reload, boardLive = false }) {
  ...
  // @@@ the shelf card OUTRANKS both console surfaces ([[archive]]). Opening a shelved session should answer
  // "this is filed away — want it back?" in one button, not drop you into a terminal you deliberately put out
  // of sight. It is the only panel shown, so relaunch stands down and folds its button into the card instead:
  // one card, one decision, and an offline shelved session still gets its restore without a second surface.
  const shelvedSel = !!selSession?.archived
  const showRelaunch = !shelvedSel && !terminalFree && noLivePane && selSession?.status !== 'queued'
          ...
          >
              ...
              >
                ...
                {[...opened].map((id) => {
                  ...
                  const shown = id === active && !shelvedSel
                  return (
