...
test('codex worktree materialize plants the .codex anchor + unconditional project trust + per-hook hashes', { skip: !gitAvailable() && 'git not available' }, () => {
  ...
  assert.ok(!existsSync(join(wt, '.claude', 'settings.json')), 'nested worktree does not duplicate the main checkout Claude dispatcher')
  ...
  assert.equal(userClaude.permissions.allow[0], 'Bash(git:*)', 'a user-owned nested Claude settings file survives migration')
  assert.equal(userClaude.hooks.PreToolUse[0].hooks[0].command, 'user-owned-hook', 'user hook survives migration')
