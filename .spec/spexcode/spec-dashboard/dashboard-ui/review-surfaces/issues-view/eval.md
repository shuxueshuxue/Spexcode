...
scenarios:
  ...
  - name: composer-mention-autocomplete
    ...
    description: >-
      ...
      launcher, and read the durable token; clear, choose the synthetic `@parent` row and then a session, and
      read that token; clear, type `[[` (and a partial id), pick, read the insertion;
    ...
    expected: >-
      ...
      fork): `@` lists retained sessions including offline rows plus the two synthetic action doors, and a
      session pick inserts `@<id> ` (trailing space) while `@new` opens the same launcher list as New
      Session and inserts `@new:<launcher> `, and `@parent` re-opens the same session rows and inserts
      `@parent:<full-id> `. A door is never itself a referent. `[[`
