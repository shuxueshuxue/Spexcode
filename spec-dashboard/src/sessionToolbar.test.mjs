...
test('session toolbar keeps the Terminal tab, adds resource tabs, then Eval and its adjacent picker', () => {
  ...
  assert.match(source, /activeResource\?\.kind === 'file'[\s\S]{0,240}data-resource-action="refresh"[\s\S]{0,160}refreshResource\(activeResource\)[\s\S]{0,360}data-resource-action="download"[\s\S]{0,220}downloadFile\(activeResource\.sessionId, activeResource\.value\)[\s\S]{0,360}data-resource-action="copy"[\s\S]{0,180}copyFilePath\(activeResource\.value\)/)
  assert.match(source, /icon="x"[\s\S]{0,160}closeResource\(tab\)/)
  assert.match(source, /uiCmds\.filter\(\(c\) => c\.button && \(!activeResource && \(!terminalFree \|\| c\.name !== 'merge'\)\)\)/)
...
test('command availability, icons, toolbar tools, and typed twins remain one registry result', () => {
  ...
  assert.match(source, /uiCmds\.filter\(\(c\) => c\.button && \(!activeResource && \(!terminalFree \|\| c\.name !== 'merge'\)\)\)[\s\S]*?\.sort\(\(a, b\) => \(a\.anchor === 'right' \? 1 : 0\) - \(b\.anchor === 'right' \? 1 : 0\)\)[\s\S]*?\.map/)
