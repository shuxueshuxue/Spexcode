...
- **Reparent, never rewrite.** Move a node by `git mv`-ing its folder; its id (= folder basename), its `[[links]]`, its `code:` governance, and its `measurement contract` all ride along untouched. If a child needs its body edited to belong in a group, it doesn't belong.
