...
scenarios:
  - name: real-gitlab-listing
    description: >
      From a repo whose origin remote is a GitLab host (the reference target: adopter-a at
      dev.innerdev.cn, reached from the machine that holds its PAT in git's credential store, with
