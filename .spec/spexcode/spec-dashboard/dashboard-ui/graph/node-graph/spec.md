...
A re-plot is one **connected structural motion**, not a node animation laid over already-finished edges. Persisting tiles travel through graph coordinates and React Flow derives their edges from those same coordinates on every frame, so an edge stays attached at both ends throughout the move. A newly revealed child unfolds outward from its parent's outgoing side while its edge grows with it; a removed branch and its edges leave atomically. The first paint and a data refresh with unchanged geometry are already settled and never replay the motion. Reduced-motion preference makes every structural update atomic.
