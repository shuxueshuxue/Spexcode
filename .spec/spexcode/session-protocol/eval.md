...
scenarios:
  - name: installed-sqlite-package-contract
    ...
    code: packages/session-protocol/src/index.ts
    related: packages/session-protocol/package.json
  ...
  - name: fifo-idempotency-and-retirement
    ...
    code: packages/session-protocol/src/index.ts
    related: packages/session-protocol/src/engine.test.ts
...
Each scenario names the implementation it actually exercises. That is the SQLite package, which is what these
scenarios were written for and what they became measurable against; anchoring them to the legacy entry would hang
every reading on a freshness axis the measurement never touches. The node itself still governs the legacy entry,
because which implementation is authoritative for the product is a cutover decision, not a measurement decision.

`node scripts/m1-conformance.mjs` is the harness: it packs the packages, installs them into a clean consumer
outside this repository, and refuses to proceed if any resolution escapes that consumer's own `node_modules`.
