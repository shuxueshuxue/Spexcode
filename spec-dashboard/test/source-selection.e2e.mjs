...
try {
  ...
  const prose = page.locator('.specview-prose .doc-body').first()
  const proseBox = await prose.boundingBox()
  assert.ok(proseBox, 'spec prose has a right-click surface')
  await page.mouse.click(proseBox.x + 12, proseBox.y + 12, { button: 'right' })
  const nodeActions = page.locator('.pa-node-group .pa-act')
  await nodeActions.first().waitFor({ state: 'visible', timeout: 10_000 })
  assert.deepEqual(await nodeActions.allTextContents(), ['Send current node to a session', 'Copy node link'],
    'plain prose right-click exposes current-node actions without a text selection')
  await page.mouse.click(proseBox.x + 240, proseBox.y + 12)
  await page.waitForTimeout(50)
  assert.equal(await page.locator('.pa-node-group').count(), 0, 'clicking outside dismisses the current-node menu')
  await page.mouse.click(proseBox.x + 12, proseBox.y + 12, { button: 'right' })
  await nodeActions.first().waitFor({ state: 'visible', timeout: 10_000 })
  await page.mouse.click(proseBox.x + 240, proseBox.y + 12, { button: 'right' })
  await page.waitForTimeout(50)
  assert.equal(await page.locator('.pa-node-group').count(), 0, 'a right-click outside dismisses the old current-node menu')
  ...
  assert.equal(await actions.count(), 0, 'left-button selection release does not open the action group')
  assert.equal(await page.locator('.srcview-select-action').count(), 0, 'the old one-button affordance is gone')
  await page.screenshot({ path: join(out, 'm4-source-selection-no-actions.png'), fullPage: true })
  await page.keyboard.press('Escape')
  await page.waitForTimeout(50)
  ...
  await page.waitForTimeout(100)
  assert.equal(await popupActions.count(), 0, 'popup left-button selection release does not open the action group')
  await page.mouse.click(bodyBox.x + Math.min(bodyBox.width - 2, 140), bodyBox.y + bodyBox.height / 2, { button: 'right' })
