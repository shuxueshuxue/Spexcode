...
scenarios:
  - name: evals-page-master-detail
    ...
    description: >
      ...
      two-column grid, the left [[evals-feed]] list with its filter chipbar (NO tab switcher), the right
      detail pane. Select an eval row and read what the RIGHT pane renders; drive j/k in the feed and read
    ...
    expected: >
      ...
      the LEFT column is the evals feed (latest reading per scenario, its own kind chips in a sticky head)
      with NO Evals|Threads switcher; the RIGHT pane is the full-height [[event-detail]] of the selection.
      Selecting an eval row renders it in the RIGHT pane as the event detail (media/note + the A/B strip +
      the remark thread) — selection IS detail, no in-place expansion in the list. j/k move ONE selection
      in the feed and the detail follows. A direct reload at #/evals opens on the Evals page (hash routing
      intact — no flash through the graph). Zero loss = the Evals page is a real top-level route with its
      own master-detail, peer to the graph, the board, and the Issues page.
---
