...
scenarios:
  - name: one-grammar-every-control-verb
    ...
    description: >-
      ...
      shape naming ONE session: its full id, an id-prefix, its branch, and the reference-sigil form
      (`@<branch>`). Then the failure shapes: a prefix that hits TWO sessions (ambiguous), and a selector that
      hits none. Also confirm the comma-list convention on a list verb: `spex session ls a,b` == `a b`.
    expected: |
      All four single-target shapes (full id · id-prefix · branch · `@<branch>`) resolve to the SAME
...
prefix/branch. Measured YATU through a real control verb (`spex session show`) against a live backend —
