...
Before constructing history indexes, lint checks the repository's adoption boundary. An untracked `.spec/`
file or `spexcode.json` is an **integrity error** naming the untracked project source of truth and the
ordinary `git add .spec spexcode.json` repair. Harness delivery files such as `.codex/`, `.claude/`, and
`AGENTS.md` are machine-local and are not part of this check. A staged candidate is judged from its candidate
tree, so staged adoption data is valid; an untracked seed cannot masquerade as a clean graph.
