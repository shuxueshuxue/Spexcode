...
test('Stop teaching names every declaration face without changing the gate', () => {
  ...
  assert.match(stopGate, /done --propose close.*task finished, work landed.*worktree no longer needed: propose human close/)
  ...
  assert.match(stopGate, /done --propose merge \(review; ONLY clickable merge\).*nothing \(done; no merge\).*close \(close-pending; no posted artifact waiting\).*park \(parked; managed watch delivery or real background wake-up\).*ask \(asking; human reply or posted artifact review\)/)
})
