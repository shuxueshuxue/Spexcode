...
Those writes also pass through [[maintenance-lease]] after normal authentication and project binding. The
scoped `session maintain` wrapper alone holds the lease bearer and carries it in the dedicated maintenance
header; nested Spex clients inherit only anonymous broker FDs and ask that wrapper to relay exact allowed
operations. The bearer never authenticates the caller or rides selector/query/argv/env/output. Ordinary callers
receive structured `maintenance_active` before any remote side effect, while reads continue through the same
endpoint ladder.
