...
test('an empty diff names which of the three branch states is true and prints complete git identities', () => {
  ...
  assert.match(diff, /const currentKey = current \? entryKey\(current\) : ''/)
  assert.match(diff, /const registerCurrentView = useCallback\(\(path, view\) => registerView\(currentKey, view\), \[registerView, currentKey\]\)/)
  assert.match(diff, /onView=\{registerCurrentView\}/)
...
test('session row focus is navigation-only', () => {
  ...
  assert.match(view, /const route = \{ page: 'sessions', param: id, query: null \}[\s\S]*?return scope\.open\(route\)/)
