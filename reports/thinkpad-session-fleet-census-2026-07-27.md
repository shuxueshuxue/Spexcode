...
| --- | --- | ---: | --- | --- |
| `206ab57c-2906-48c3-8084-e9eca97eb478` | archived, parked, online / none | 1 / 0 / `3bf641ed` | 0; Claude has no Codex thread id; session owner RSS 365.8 MiB | Parent of `9a1eb40b`; note carries the CR dashboard handoff plus `!1657`, `!1658`, `http://10.253.209.97:5173`, and `http://100.99.97.58:5173`. Keep until the external dashboard/W23 handoff is acknowledged. Its only commit `0dca7999` is patch-equivalent to main, but the live owner and child lineage remain. |
...
| `b5acb442-ebda-437e-b399-c35b93f8d3ae` | archived, asking, offline / none | 0 / 0 / `934bf7c5` | 0; thread `019f9e91-8454-7a23-b4b7-0650140bd665`, record-only/no loaded ref | Strict-three adjudication was accepted, but outbound acknowledgment timed out and diagnostics remain explicitly awaited. Human receipt is unproven. |
...
| --- | --- | ---: | --- | --- |
| `9a1eb40b-b9cc-4e12-a89d-fe4c8fb370ad` | archived, parked, online / none | 0 / 0 / `62d7522b` | 0; thread `019f9481-71bf-7402-a30b-cb094b3b25f8`, record-only/no loaded ref | Child of `206ab57c`; external MBP adopter-a W23 lane, with GitLab context and a remote product branch not represented by local Spex commits. Parent note says replay and real YATU are still pending. Receiving owner must acknowledge the remote commits and evidence before close. |
...
| `484a29ac-8241-4cb3-9778-2b85b73964ae` | archived, done / nothing | 18 / 0 / `4c7d2dfa` | 17 diff entries; thread `019f9d76-27bd-70f1-98ae-7d1705380531`, record-only/no loaded ref | Unique session-console latency implementation/evidence remains branch-only; `conflictsWithMain=true`. Unique commits include `078b7d18`, `b2044ab6`, `1db4348a`, `b72d911b`, `d0c6e456`, `9ca9574b`, `e5d4842f`, `23398f11`, `f3e9b8f9`, `14ef974e`, `b38bc892`, `a9775c01`, `42454e09`, `778c80eb`, plus three merge commits and `ca11c814` (Spec-OK). Preserve the code/spec/eval/test files and decide whether to replay or intentionally supersede. |
...
| `0e6a1bc4-fc35-4f3d-9c78-20d7b71991a5` | archived, parked, offline / none | 0 / 0 / `95dd52f8` | 0; thread `019f9eca-64de-7ed0-a5e4-bdce5e4f9591`, record-only/no loaded ref | Session-eval lane still reports one real CLI contract failure (blind-before-measured plus inherited divider) after 13/15 checks passed. It has no branch delta, but the unfinished measured result needs a named owner before close. |
...
| `44da65b5-f338-4564-9c9e-c9fd2c1bde75` | archived, done, offline / nothing | 0 / 0 / `dc76dd4b` | 0; thread `019f9f16-4e6f-7812-968d-01e4fd9744ce`, record-only/no loaded ref | Production recovery landed at `dc76dd4b`; no branch delta. |
...
| `25745879-20cc-467c-8ea0-c7104435b145` | archived, done, online / nothing | 0 / 0 / `6b70a7a8` | 0; thread `019fa0c8-0d6a-7e41-bb5a-4365ec026bd2`, record-only/no loaded ref | Production runtime milestone applied at main `6b70a7a8`; no branch delta. Archive-to-offline is required before close because the row is still online. |
...
| `093e1d7e-5bdf-4963-bf4f-b5c11cfbc550` | done, online / nothing | 0 / 0 / `6b70a7a8` | 0; thread `019fa157-4324-7040-91a8-63eca6e2b326`, record-only/no loaded ref | Source-of-truth audit complete with no branch delta. Its successor checklist remains historical note content, not an unowned commit. |
...
| `3d484355-4635-4e33-b5f0-e5cdb1de5dfc` | review, online / merge | 0 / 0 / `304814a0` | 0; thread `019fa204-cbaf-70e1-8c50-1adeae42e15d`, record-only/no loaded ref | The prior MBP ledger is already on main at `304814a0`; this row has no branch delta. Do not replace that historical report; close after the current archive fix. |
...
`2,032.1 MiB`, CPU `12.9%`, healthy `refCount=67`, host swap `2,048 MiB` used, and `9` findings. The
manager's resource census supplied a close-first set of 12 low-risk rows; because that message did not carry
the IDs, this ledger uses the first twelve clean/no-unique rows as the provisional set:
`c115afbe`, `b2aa74d1`, `44da65b5`, `296fa2e5`, `b3e1d382`, `eed69846`, `518ceed0`, `eb8126bd`, `25745879`,
`dc9077ab`, `3adfb668`, and `093e1d7e`. Reconfirm that exact set from the manager immediately before any
mutation. Defer `484a29ac`, `5c7c1125`, and `3d484355` as explicitly reported, regardless of their current
branch deltas.
...
4. After a fresh probe, close only the manager-confirmed close-first set one at a time. Defer
   `484a29ac`, `5c7c1125`, and `3d484355` until their reported review/lineage conditions are explicitly cleared.
   The remaining `CLOSE-AFTER-ARCHIVE-FIX` rows (`7093c143`, `d9d3875c`, `a2ee3a8e`, and `ae16f8e5`) follow only
   after the same archive/offline and exact-owner checks.
