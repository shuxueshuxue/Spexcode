...
新行为是好是坏,只能说 spec 已经描述不了代码了。而这恰恰是它值得跑的原因:图里那个 commit 在同一次
重命名里更新了另外七个 spec、漏掉了这一个——正常的工作,也正是机械检查逮得住的那类遗漏。
...
浏览器标签页管全部仓库。它的 `/projects` 页就是管理面——在浏览器里浏览到一个目录、就地采纳、启动或
唤醒它的后端、设置访问密码;后端不依赖 gateway 存活,先后顺序无所谓。然后开始长树:在
