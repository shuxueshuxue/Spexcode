...
scenarios:
  - name: session-actions-live-in-shell-slot
    ...
    expected: >-
      ...
      rename/close live on the tab's context menu, not in the slot. An unavailable merge exposes the exact
      reason in its accessible label. The slot's members are not all buttons: the Eval door is a real
