...
**Row 1 — identity & recency:** `status dot · title`, with the **last-edited age** flushed to the right edge — bare ("3h ago"), no label, absent when there is no committed history — so the title line stays clean. The tile's `node-dot` (the session-row face has no dot — it leads with its avatar) shows the backend-**derived** four-state (see [[spec-node-states]]): green merged, orange active (pulsing), yellow drift, grey pending.

**Row 2 — marks & people:** everything denser lives below the title line: the `version`, the badge cluster — drift's commits-ahead `⚠N`, open issues, scenario count — and the worktree pending-op glyphs stamped in the author session's colour (`+` added, `~` edited, `✕` deleted, `→` moved; dashed ring while uncommitted, an `added`-only node draws as a translucent ghost). Then the node's *live editors* — the sessions whose pending ops currently touch it (the live overlay, never the historical `session` trailer) — one **avatar** each ringed by liveness and capped with `+N`.
