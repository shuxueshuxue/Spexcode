...
z-code discovers Claude-compatible command hooks from `.zcode/settings.json`. Its shim binds the five lifecycle events that SpexCode needs — `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop` — with each command dispatching the baked `zcode` harness id. That path is measured, not assumed: a launched z-code worker's `Stop` gate fired through exactly this file, and the taught note landed in the session store in the same millisecond as the `awaiting` status write. z-code has no `Notification` or `StopFailure`, so the existing Claude-only idle state is unavailable and no substitute event is fabricated. The payload is Claude-shaped, so the shell mirror routes `zcode` through the existing Claude-family parser rather than adding a second parser.
