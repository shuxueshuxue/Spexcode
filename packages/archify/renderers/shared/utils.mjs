...
export function applyTemplate(template, {
  ...
  return templateWithI18n
    ...
    // SpexCode's one extension: the author's note sits under the diagram and above the cards, and it stays
    // visible in embed mode (the cards do not) — it is the one line meant for whoever is looking at the picture.
    .replace(CARDS_SLOT_RE, () => `${typeof note === 'string' && note.trim() ? `    <p class="diagram-note">${esc(note)}</p>\n` : ''}${cards}`)
