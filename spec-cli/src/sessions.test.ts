...
test('a failed creation-time materialize is reported loud and stamped on the record note', () => {
  ...
  try {
    const rec: SessRec = {
      ...
      harness: 'claude', harnessSessionId: null, archived: false, launcher: 'reclaude', launchCmd: 'claude', launchOwner: null,
    }
...
test('owned queues are public-authority leased and raw-state fenced from legacy drainers', () => {
  ...
  const base: SessRec = {
    ...
    createdAt: 1, harness: 'codex', harnessSessionId: null, archived: false, launcher: 'codex', launchCmd: 'codex',
