...
test('session tree drag moves the whole row and can detach a nested session', () => {
  ...
  assert.match(source, /startSessionDrag\(e, s, \{[\s\S]{0,320}lead, selecting, picked: isPicked/)
  assert.match(source, /className="si-session-drag-ghost"[\s\S]{0,900}<SessionRow s=\{sessionDrag\.session\}[\s\S]{0,160}lead=\{sessionDrag\.appearance\.lead\}/)
  assert.match(source, /sessionDrag\.appearance\.selecting && <span className=\{`si-check/)
