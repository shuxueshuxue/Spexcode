...
- **next lifecycle change** — a governed parent automatically installs the child's `parent` watch source after
  creation, registering durable send-backed delivery for every declared state except routine working.
  `spex session reparent <id> --to <parent>` moves that source; `watch cancel` affects only an independent manual watch. A caller without a governed session
