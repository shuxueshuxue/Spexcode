...
scenario('multiple watchers receive one ordered stream', () => {
  ...
  const first = app.protocol.listPending('w1')
  const second = app.protocol.listPending('w2')
  assert.deepEqual(first.map(message => message.kind), ['session.state.changed.v1', 'session.state.changed.v1'])
  assert.deepEqual(second.map(message => message.kind), first.map(message => message.kind))
  assert.ok(first[0].enqueueSeq < first[1].enqueueSeq)
  assert.ok(second[0].enqueueSeq < second[1].enqueueSeq)
  app.close()
})
