...
//   2. boardCache's scope has a DOMAIN, not just a dirty bit: a 'sessions'-scoped invalidation splices (and so
...
// isolating env FIRST, then `await import()` board/boardCache so every git/spec/tmux resolution lands on the
// fixture — never the real repo or the real ~/.spexcode. Node runs each test FILE in its own process, so the
// module-level caches (repoRoot, gitCommonDir, boardCache's cached/dirty/inflight) are ours alone.
...
let cache: typeof import('./boardCache.js')
...
if (gitOk) {
  ...
  cache = await import('./boardCache.js')
...
// ---------------------------------------------------------------------------------------------------------
// 2. SCOPE ESCALATION (boardCache) — one sequential test so getBoard's module state stays order-deterministic.
