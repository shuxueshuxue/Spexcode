...
try {
  ...
  await page.keyboard.press('Escape')
  // ONE LIST OWNS DISCLOSURE, because there is one list ([[dock-modes]]). The window used to carry a second,
  // thinner session list inside the finding dock, with its own copy of this row/header ownership to keep in
  // step; asking for the sessions projection now brings the SAME forest this file already exercised above,
  // on whatever route the reader is on. The retired surface must not come back.
  await page.evaluate(() => { localStorage.setItem('spexcode.dock', '1'); localStorage.setItem('spexcode.dockMode', 'sessions') })
  await page.reload({ waitUntil: 'domcontentloaded' })
  await page.locator('.app > .si-list').waitFor({ state: 'visible', timeout: 20_000 })
  assert.equal(await page.locator('.dock-session-list, .dock-session-body').count(), 0, 'the retired thin session list is gone')
  const framedParent = page.locator('.app > .si-list .sess-tree-row:has(> .sess-fold-control)').first()
  const framedPod = framedParent.locator('> .sess-fold-control')
  const framedBody = framedParent.locator('> .si-item')
  const framedRowsBefore = await visibleRows(page, '.app > .si-list .si-item')
  assert.equal(await framedBody.getAttribute('aria-expanded'), null, 'the row is not the disclosure control')
  if (await expanded(framedPod) === 'false') {
    await framedPod.click()
    assert.equal(await expanded(framedPod), 'true')
    assert.ok(await visibleRows(page, '.app > .si-list .si-item') > framedRowsBefore)
  }
  assert.equal(await page.locator('.app > .si-list button button').count(), 0)
  record('Framed forest', 'row/header ownership', 'row=false,pod=true')
  await page.screenshot({ path: `${OUT}/framed-forest-disclosure.png` })
