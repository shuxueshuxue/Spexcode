...
test('harness target addition persists targets, materializes, and is exposed through the admin route', async () => {
  ...
  const before = JSON.parse(readFileSync(join(repo, 'spexcode.json'), 'utf8'))
  ...
  assert.deepEqual(JSON.parse(readFileSync(join(repo, 'spexcode.json'), 'utf8')).harnesses, ['claude', 'codex'])
  assert.equal(added.launcher?.harness, 'codex')
  assert.equal(typeof added.launcher?.cmd, 'string')
  const after = JSON.parse(readFileSync(join(repo, 'spexcode.json'), 'utf8'))
  ...
  assert.deepEqual(JSON.parse(readFileSync(join(root, 'spexcode.json'), 'utf8')).harnesses, ['claude', 'codex'])
  ...
  assert.deepEqual(JSON.parse(readFileSync(join(pluginRepo, 'spexcode.json'), 'utf8')).harnesses, [{ plugin: '.adopter-a' }])
...
test('harness target addition refuses a missing persisted selection instead of inventing one', async () => {
  ...
  execFileSync('git', ['init', '-q'], { cwd: repo })
  writeFileSync(join(repo, 'spexcode.json'), '{}\n')
  await assert.rejects(addHarnessTarget(repo, 'codex'), /no "harnesses" field.*spex init --harness/i)
  assert.deepEqual(JSON.parse(readFileSync(join(repo, 'spexcode.json'), 'utf8')), {})
})
