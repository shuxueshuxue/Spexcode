...
scenarios:
  - name: proxy-target-precedence
    ...
    description: >-
      ...
      `API_URL` but a `.spec/spexcode.json` carrying `dashboard.apiUrl=<backend B>` reachable by walking up from
    ...
    expected: >-
      ...
      `.spec/spexcode.json` `dashboard.apiUrl` found by walking up from cwd is used (case 2 → backend B answers).
