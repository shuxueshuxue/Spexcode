...
try {
  ...
  assert.equal(await archivedChat.locator('.m-input').count(), 0, 'archive index row did not hide the read-only composer input')
  assert.equal(await archivedChat.locator('.m-composer-tools').count(), 0, 'archive index row did not hide the read-only composer controls')
