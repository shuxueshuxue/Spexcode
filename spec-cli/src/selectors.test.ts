...
function mk(id: string, node: string | null, branch: string | null): Session {
  return {
    id, node, branch, label: node || branch || id, headline: node || branch || id, raw: { name: null, title: null }, path: `/wt/${id}`, parent: null, harness: 'claude', capabilities: { headless: false, messageStream: false }, launcher: null,
