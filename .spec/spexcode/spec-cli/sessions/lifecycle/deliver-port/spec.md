...
verified fix, two readings on one the former measurement core scenario). So the trunk driver's admission does not invent
...
justifies the merge (the the thread detail is where a reviewer flips A↔B). This stays a *reference*, not a copy:
