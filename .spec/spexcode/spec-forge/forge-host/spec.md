...
code:
  ...
  - spec-forge/src/drivers.ts#forgeDriverFor
---
# forge-host
...
board/issues slices said `host: 'github'` literally — so a repo whose remote is not GitHub (adopter-a's
...
- `forge.host` in `.spec/spexcode.json` / `.spec/spexcode.local.json` (local wins, same layering as every other
...
- the **origin remote's hostname** (`git remote get-url origin`, both URL and scp-like shapes): a
  ...
  the common self-hosted forge shape (e.g. `dev.innerdev.cn`), deliberately a guess because a wrong
