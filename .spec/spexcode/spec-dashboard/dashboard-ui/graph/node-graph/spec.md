...
A re-plot is an **atomic structural update**, never an animation. Newly revealed tiles and their edges appear together at final geometry; persisting tiles and edges move to their new slots in the same render; removed branches leave together. The graph adds no entrance, position, edge, focus-dimming, or camera transition, so there is no intermediate frame in which topology is visually disconnected or still travelling. The camera still follows a navigation target as described above, but snaps straight to its framing.
