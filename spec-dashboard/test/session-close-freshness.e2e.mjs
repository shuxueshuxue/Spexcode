...
try {
  ...
  const tabLabel = page.locator(`.si-document [role="tab"][data-tab-key="#/sessions/${sessionId}"] .tab-label`)
  await tabLabel.waitFor({ state: 'visible', timeout: 5_000 })
  assert.equal((await tabLabel.textContent())?.trim(), 'close freshness target', 'closed session tab keeps its title')
  await page.goto(`${base}/#/graph`, { waitUntil: 'domcontentloaded' })
  const shellTabLabel = page.locator(`.app-main > .tabstrip [role="tab"][data-tab-key="#/sessions/${sessionId}"] .tab-label`)
  await shellTabLabel.waitFor({ state: 'visible', timeout: 5_000 })
  assert.equal((await shellTabLabel.textContent())?.trim(), 'close freshness target', 'closed session title survives leaving Sessions')
  await page.goto(`${base}/#/sessions/${sessionId}`, { waitUntil: 'domcontentloaded' })
