...
status: active
hue: 280
desc: The plugin BUNDLE emitter — render the whole SpexCode system into one self-contained Claude-plugin bundle dropped into the spexcode.json-named folder, so a plugin host (adopter-a/Claude/future Codex) reads it natively with zero --append-system-prompt.
code:
...
and (being exclusive) leaves every native harness to be pruned. This node is that missing EMITTER: it renders
...
The pieces map from the same [[surface]] nodes the native path renders, but through a plugin host's seams:
...
- **the contract** (the [[harness-delivery]] assembly — `docs/AGENT_GUIDE.md` followed by the `surface: system`
  ...
  **SessionStart hook that emits `hookSpecificOutput.additionalContext`** (the harness-neutral injection
  ...
  can't take. The additionalContext JSON is encoded at RENDER time into `hooks/contract-context.json`, so the
...
- **the hooks** reuse the SAME `dispatch.sh` wiring as the natives — `dispatch.sh` + its shell mirror
  `harness.sh` are copied verbatim into `hooks/`, and `hooks/hooks.json` (the Claude/adopter-a shape
  ...
  (adopter-a/Claude share Claude's tool names + `file_path`) via its default case, no separate arm. The per-event
...
artifact (its `hooks.json` bakes this install's `SPEX` path), so it joins the managed `.gitignore` block — like
