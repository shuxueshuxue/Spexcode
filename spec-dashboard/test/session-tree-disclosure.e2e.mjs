...
try {
  ...
  assert.equal(await expanded(interfacePod), 'false', 'Ctrl+Right must remain native and not disclose the parent')
  mark('toggle current parent with primary Shift+E')
  await page.keyboard.press('Control+Shift+E')
  assert.equal(await expanded(interfacePod), 'true', 'Ctrl+Shift+E must expand the selected parent')
  assert.equal(await page.locator(`.si-item[data-sid="${child.id}"]`).count(), 1)
  await page.keyboard.press('Control+Shift+E')
  assert.equal(await expanded(interfacePod), 'false', 'Ctrl+Shift+E must collapse the selected parent')
  assert.equal(await page.locator(`.si-item[data-sid="${child.id}"]`).count(), 0)
  record('SessionInterface', 'primary Shift+E toggles current parent', 'false→true→false')
  ...
  await page.keyboard.press('Control+ArrowRight')
  assert.ok(await leafCommand.evaluate((input) => input.selectionStart) > 0, 'Ctrl+Right must keep native input navigation')
  await page.keyboard.press('Control+Shift+E')
  assert.equal(await leafCommand.inputValue(), 'alpha beta', 'a leaf must not consume primary Shift+E')
  record('SessionInterface', 'leaf keeps native primary shortcut', true)
