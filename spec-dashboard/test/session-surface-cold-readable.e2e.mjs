...
try {
  ...
  const assertSurface = async (id, state, expectedLine, expectedAction) => {
    ...
    assert.equal(await footer.locator('.m-input').count(), 0, `${state} composer input should be hidden`)
    assert.equal(await footer.locator('.m-composer-tools').count(), 0, `${state} composer controls should be hidden`)
  ...
  await page.locator('.m-seam-inset .tx-flow').waitFor({ state: 'visible' })
  assert.equal(transcriptRequests.get(archivedId), 1, 'expanded seam did not issue exactly one transcript request')
  assert.ok((await page.locator('.m-seam-inset .tx-flow').innerText()).includes('persisted transcript turn'), 'archived transcript turn did not render after worktree removal')
  const tool = page.locator('.m-seam-inset .tx-tool').first()
  assert.equal(await tool.locator('.tx-tool-out').count(), 0, 'tool output was expanded by default')
  await tool.locator('.tx-tool-row').click()
  ...
  step('archived timeline readable with hidden composer controls and terminal control')
