...
map. Names are lowercase kebab-case and definitions provide a component: a callable, or one of React's tagged
component objects (`lazy`, `memo`, `forwardRef`) — any other object fails closed. Registration fails closed
...
The registry also hands the shell its one frozen route contract — is this address a registered view, is it a
document, is it resident — so tab and document policy has no second map to drift from ([[view-registry]]).
