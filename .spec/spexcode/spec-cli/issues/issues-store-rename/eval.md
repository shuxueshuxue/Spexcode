...
scenarios:
  - name: migrate-on-first-touch
    ...
    related: [packages/spec-core/src/git.ts]
    description: >-
      ...
      read (`spex issue ls --all`) — then inspect the trunk.
    expected: >-
      ...
      pre-rename history, and `spex issue ls --all` reads the thread — including every reply — identically to
  ...
  - name: concurrent-first-touch
    ...
    description: >-
      ...
      very first touch after the update (a burst of `spex issue open`/`spex issue ls`).
    expected: >-
...
proof drives the same first-touch an updated deployment hits — a plain `spex issue ls`/`spex issue open` — and
