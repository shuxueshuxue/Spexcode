...
test('mobile session detail retains the aligned TimelineChat composer', () => {
  ...
  // the footer still speaks the Command Box's kind and the note reply channel; what it sends is the composed
  // message, so a widget's pending block ([[widgets]]) rides the same one send rather than a second route
